...
dependency:micronaut-langchain4j-agentic[groupId=io.micronaut.langchain4j]
...
[.lang-python]
NOTE: In Python an agentic service is an abstract class (`abc.ABC`) whose methods are declared with `...` bodies; the decorators mirror the LangChain4j annotations.

[.lang-python]
WARNING: The Python compiler turns a Python `@AgenticService` into a Micronaut bean, but LangChain4j builds the agents reflectively from the Java interface, which the generated Python bean does not expose yet. Python agentic services therefore cannot be invoked at runtime with the current Python compiler release; the Python snippets document the intended programming model.
