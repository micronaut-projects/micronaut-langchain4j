...
public final class TestAgentBuilderListeners {
    ...
    @Singleton
    @Requires(env = "customizer")
    static final class SequencialBuildListener
        implements BeanCreatedEventListener<WorkflowService<?, ?>> {
        private final TestCustomizerTracker tracker;

        @Inject
        SequencialBuildListener(TestCustomizerTracker tracker) {
            this.tracker = tracker;
        }

        @Override
        public WorkflowService<?, ?> onCreated(
            @NonNull BeanCreatedEvent<WorkflowService<?, ?>> event) {
            tracker.increment();
            return event.getBean();
        }
    }
